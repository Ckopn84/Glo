/*jshint esversion: 6 */ // что бы не было ошибки: 'arrow function syntax (=>)' is only available in ES6 (use 'esversion: 6')
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const search = document.querySelector('.search'),
        ...
        wishlistConter = wishlistBtn.querySelector('.counter'),
    ...
    const renderBasket = goods => {
        ...
        if (goods.length) {
            goods.forEach(({id, title, price, imgMin}) => {
                cartWrapper.append(createCardGoods(id, title, price, imgMin));
            });
        } else {
            cartWrapper.textContent = '<div id="cart-empty">Ваша корзина пока пуста.</div>';
        }
    };
    ...
    const calcTotalPrice = goods => {
        let sum = goods.reduce((accum, item) => {
            return accum + item.push * goodsBasket[item.id];
        }, 0);
    ...
    const checkCount = () => {
        wishlistConter.textContent = wishlist.length;
    ...
    const openCart = event => {
        ...
        document.addEventListener('keyap', closeCart);
    ...
    search.addEventListener('click', searchGoods);
